...
function initializeControls() {
    ...
    updateButton.addEventListener('click', async () => {
        if (selectedBands.length > 0) {
            const data = await initVisualization();
            currentData = data;
        }
    });

    // Add view control buttons
    const separateBtn = document.getElementById('separateView');
    const overlapBtn = document.getElementById('overlapView');

    separateBtn.addEventListener('click', () => {
        if (selectedBands.length > 0 && currentData) {
            createStreamGraph(currentData, false);
        }
    });

    overlapBtn.addEventListener('click', () => {
        if (selectedBands.length > 0 && currentData) {
            createStreamGraph(currentData, true);
        }
    });
}
...
function createStreamGraph(data, isOverlapped = false) {
    ...
    const height = isOverlapped ? 400 : selectedBands.length * 100;
    ...
    const y = d3.scaleLinear()
        ...
        .range([isOverlapped ? height - margin.top : 60, 0]);
    ...
    const ridges = svg.selectAll(".ridge")
        ...
        .join("g")
        .attr("class", "ridge")
        .attr("transform", (d, i) => isOverlapped ? 
            `translate(0, 0)` : 
            `translate(0, ${i * 70})`);
    ...
    ridges.append("path")
        ...
        .attr("opacity", isOverlapped ? 0.5 : 0.7)
        .attr("stroke", d => d3.rgb(color(d.name)).darker(0.5))
        .attr("stroke-width", 1);

    // Add x-axis based on view mode
    if (!isOverlapped) {
        ridges.append("g")
            .attr("class", "x-axis")
        ...
        // Add single x-axis for overlapped view
        svg.append("g")
            .attr("class", "x-axis")
    ...
    ridges.selectAll("path")
        .on("mouseover", function() {
            d3.select(this)
                .attr("opacity", isOverlapped ? 0.8 : 1)
                .attr("stroke-width", 2);
        })
